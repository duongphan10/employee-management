...
public class FamilyRelationshipValidator {
    ...
    public void checkPermissionGet(int id, int userId) {
        StoredProcedureQuery storedProcedureQuery = entityManager.createStoredProcedureQuery("sp_CheckPermissionGetFamilyRelationship")
    ...
    public void checkPermissionCreate(int userId, EmployeeDto employeeDto) {
        if (employeeDto.getCreatedBy() != userId)
            throw new ForbiddenException(ErrorMessage.FamilyRelationship.FORBIDDEN_CREATE);
        if (employeeDto.getStatusId() != DataConstant.Status.NEW.getId())
            throw new ForbiddenException(ErrorMessage.FamilyRelationship.FORBIDDEN_CREATE_BY_STATUS);
    }

    public void checkPermissionUpdate(int employeeId, int userId) {
        StoredProcedureQuery storedProcedureQuery = entityManager.createStoredProcedureQuery("sp_CheckPermissionUpdateFamilyRelationship")
        ...
        int result = (int) storedProcedureQuery.getOutputParameterValue("result");
        if (result == 0)
            throw new ForbiddenException(ErrorMessage.FamilyRelationship.FORBIDDEN_UPDATE);
        else {
            if (result == 1)
                throw new ForbiddenException(ErrorMessage.FamilyRelationship.FORBIDDEN_UPDATE_BY_STATUS);
        }
    }

    public void checkPermissionDelete(int employeeId, int userId) {
        StoredProcedureQuery storedProcedureQuery = entityManager.createStoredProcedureQuery("sp_CheckPermissionDeleteFamilyRelationship")
                .registerStoredProcedureParameter("employeeId", Integer.class, ParameterMode.IN)
                .registerStoredProcedureParameter("userId", Integer.class, ParameterMode.IN)
                .registerStoredProcedureParameter("result", Integer.class, ParameterMode.OUT)
                .setParameter("employeeId", employeeId)
        ...
        int result = (int) storedProcedureQuery.getOutputParameterValue("result");
        if (result == 0)
            throw new ForbiddenException(ErrorMessage.FamilyRelationship.FORBIDDEN_DELETE);
        else {
            if (result == 1)
                throw new ForbiddenException(ErrorMessage.FamilyRelationship.FORBIDDEN_DELETE_BY_STATUS);
        }
    }
